...
async def most(client, message):
    ...
    reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=True, resize_keyboard=True, placeholder="Top Most searches of the day")
    nixbotz=await message.reply_text("ɪ'ᴍ ғᴇᴛcʜɪɴɢ ᴍᴏꜱᴛ ꜱᴇᴀʀᴄʜ oғ ᴛoᴅᴀ𝚈 👩🏻‍💻")
    await nixbotz.edit_text("Pʟᴇᴀsᴇ wᴀɪᴛ...")
    await nixbotz.delete()
    await message.reply_text(f"<b>Hᴇʀᴇ ɪꜱ ᴛʜᴇ ᴛᴏᴘ ᴍᴏꜱᴛ ꜱᴇᴀʀᴄʜ ʟɪꜱᴛ 👇</b>", reply_markup=reply_markup)
...
async def trendlist(client, message):
    ...
    if not top_messages:
        await message.reply_text("ɴo ᴛᴏᴘ ᴍᴏꜱᴛ ꜱᴇᴀʀᴄʜ ᴍᴇssᴀɢᴇꜱ ғoᴜɴᴅ 🚫")
        return  
    seen_messages = set()
    truncated_messages = []
    ...
    if not truncated_messages:
        await message.reply_text("No valid top most messages found.")
        return  

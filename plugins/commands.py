...
async def start(client, message):
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        ...
        await message.reply(script.START_TXT.format(message.from_user.mention if message.from_user else message.chat.title, wish(), temp.U_NAME, temp.B_NAME), reply_markup=reply_markup, disable_web_page_preview=True)
    ...
    if AUTH_CHANNEL and not await is_req_subscribed(client, message):
        ...
        await client.send_photo(
            ...
            photo=FORCESUB_IMG,
            caption="""<b>ᴊᴏɪɴ ᴏᴜʀ ᴜᴘᴅᴀᴛᴇs ᴄʜᴀɴɴᴇʟ ᴀɴᴅ ᴛʜᴇɴ ᴄʟɪᴄᴋ ᴏɴ "ᴛʀʏ ᴀɢᴀɪɴ" ᴛᴏ ɢᴇᴛ ʏᴏᴜʀ ʀᴇǫᴜᴇꜱᴛᴇᴅ ꜰɪʟᴇ.</b>""",
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help"]:
        ...
        return
    if message.command[1].startswith("nixreff"):
            try:
                user_id = int(message.command[1].split("_")[1])
            except ValueError:
                await message.reply_text("ɪɴvᴀʟɪᴅ ʀᴇғᴇʀ!")
                return
            if user_id == message.from_user.id:
                await message.reply_text("ʜᴇʏ ᴅᴜᴅᴇ, ʏᴏᴜ cᴀɴ'ᴛ ʀᴇғᴇʀ ʏᴏᴜʀsᴇʟғ 🤣!\n\nsʜᴀʀᴇ ʟɪɴᴋ ʏᴏᴜʀ ғʀɪᴇɴᴅ ᴀɴᴅ ɢᴇᴛ 𝟷𝟶 ʀᴇғᴇʀʀᴀʟ ᴘᴏɪɴᴛ ɪғ ʏᴏᴜ ᴀʀᴇ ᴄᴏʟʟᴇᴄᴛɪɴɢ {} ʀᴇғᴇʀʀᴀʟ ᴘᴏɪɴᴛs ᴛʜᴇɴ ʏᴏᴜ ᴄᴀɴ ɢᴇᴛ 𝟷 ᴍᴏɴᴛʜ ғʀᴇᴇ ᴘʀᴇᴍɪᴜᴍ ᴍᴇᴍʙᴇʀsʜɪᴘ.".format(REFERAL_POINT))
                return
            ...
            total = REFERAL_POINT
            fromuse = refeedb.get_refer_points(user_id) + 10
            referdb.add_refer_points(user_id, fromuse)
            await message.reply_text(f"ʏᴏᴜ ʜᴀvᴇ ʙᴇᴇɴ sᴜccᴇssғᴜʟʟʏ ɪɴvɪᴛᴇᴅ ʙʏ {uss.mention}!")
            await client.send_message(user_id, f"<b>🎉 coɴɢʀᴀᴛᴜʟᴀᴛɪoɴs !</b> ʏᴏᴜ woɴ 𝟷𝟶 ʀᴇғᴇʀʀᴀʟ ᴘᴏɪɴᴛs ʙᴇcᴀᴜsᴇ ʏᴏᴜ ʜᴀvᴇ ʙᴇᴇɴ sᴜccᴇssғᴜʟʟʏ ɪɴvɪᴛᴇᴅ ☞ {message.from_user.mention}!") 
            if fromuse == REFERAL_POINT:
                await db.give_referal(user_id)
                referdb.add_refer_points(user_id, 0) 
                await client.send_message(chat_id=user_id,
                    text=f"<b>Hᴇʏ {uss.mention}\n\nYᴏᴜ ɢᴏᴛ 𝟷 ᴍᴏɴᴛʜ ᴘʀᴇᴍɪᴜᴍ sᴜʙsᴄʀɪᴘᴛɪᴏɴ ʙʏ ɪɴᴠɪᴛɪɴɢ 𝟷𝟶 ᴜsᴇʀs ❗", disable_web_page_preview=True              
                    )
                for admin in ADMINS:
                    await client.send_message(chat_id=admin, text=f"#ᴛᴀsᴋ ᴄᴏᴍᴘʟᴇᴛᴇᴅ !!\n\n\nsᴜᴄᴄᴇssғᴜʟʟʏ {total} ʀᴇғᴇʀʀᴀʟ ᴄᴏᴍᴘʟᴇᴛᴇᴅ ʙʏ ᴛʜɪs ᴜsᴇʀ:\n\n◉ ɴᴀᴍᴇ: {ni.mention}\n\n◉ ᴜsᴇʀ-ɪᴅ: {ni.id}!")	
                return
    ...
    if data.startswith("sendfiles"):
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    InlineKeyboardButton("♻️ ᴠᴇʀɪꜰʏ ♻️", url=await get_token(client, message.from_user.id, f"https://telegram.me/{temp.U_NAME}?start="))
                ],[
...
@Client.on_message(filters.command('grp_settings'))
async def grp_settings(client, message):
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply("<b>ʏᴏᴜ'ʀᴇ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪɴ!! ᴛᴜʀɴ ᴏꜰꜰ ᴀɴᴏɴʏᴍᴏᴜꜱ ᴀᴅᴍɪɴ ᴀɴᴅ ᴛʀʏ ᴛʜɪꜱ ᴀɢᴀɪɴ ᴄᴏᴍᴍᴀɴᴅ.</b>")
    chat_type = message.chat.type
...
async def deletemultiplefiles(bot, message):
    ...
    btn = [[
       ...
       InlineKeyboardButton("✔ ʏᴇꜱ, ᴄᴏɴᴛɪɴᴜᴇ ✔", callback_data=f"killfilesdq#{keyword}")
       ],[
...
async def ginfo(bot, message):
    ...
    if user.status != enums.ChatMemberStatus.ADMINISTRATOR and user.status != enums.ChatMemberStatus.OWNER and str(userid) not in ADMINS:
        ...
        if 'shortlink' in settings.keys() and 'tutorial' in settings.keys():
            ni = settings['shortlink']
            x = settings['shortlink_api']
            botz = settings['tutorial']
            return await message.reply_text(f"<b><u>ᴄᴜʀʀᴇɴᴛ  ꜱᴛᴀᴛᴜꜱ<u>\n\nᴡᴇʙꜱɪᴛᴇ : <code>{ni}</code>\n\nᴀᴘɪ : <code>{x}</code>\n\nᴛᴜᴛᴏʀɪᴀʟ : {botz}</b>", disable_web_page_preview=True)
        elif 'shortlink' in settings.keys() and 'tutorial' not in settings.keys():
            nixbotz = settings['shortlink']
            x = settings['shortlink_api']
            return await message.reply_text(f"<b><u>ᴄᴜʀʀᴇɴᴛ  ꜱᴛᴀᴛᴜꜱ<u>\n\nᴡᴇʙꜱɪᴛᴇ : <code>{nixbotz}</code>\n\nᴀᴘɪ : <code>{x}</code>\n\nᴜꜱᴇ /set_tutorial ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ꜱᴇᴛ ʏᴏᴜʀ ᴛᴜᴛᴏʀɪᴀʟ.")
        elif 'shortlink' not in settings.keys() and 'tutorial' in settings.keys():
            x = settings['tutorial']
            return await message.reply_text(f"<b>ᴛᴜᴛᴏʀɪᴀʟ : <code>{x}</code>\n\nᴜꜱᴇ  /shortlink  ᴄᴏᴍᴍᴀɴᴅ  ᴛᴏ  ᴄᴏɴɴᴇᴄᴛ  ʏᴏᴜʀ  ꜱʜᴏʀᴛɴᴇʀ</b>")
        else:
...
async def myplan(client, message):
    ...
    if data and data.get("expiry_time"):
        ...
        await message.reply_text(f"#ᴘʀᴇᴍɪᴜᴍ_ᴜꜱᴇʀ_ᴅᴀᴛᴀ :\n\n◉ ᴜꜱᴇʀ : {user}\n◉ᴜꜱᴇʀ-ɪᴅ : <code>{user_id}</code>\n◉ ᴛɪᴍᴇ ʟᴇꜰᴛ : {time_left_str}\n◉ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}")   
    else:
...
async def give_premium_cmd_handler(client, message):
    if len(message.command) == 4:
        ...
        if seconds > 0:
            ...
            expiry = data.get("expiry_time")   
            expiry_str_in_ist = expiry.astimezone(pytz.timezone("Asia/Kolkata")).strftime("%d-%m-%Y\n⏱️ ᴇxᴘɪʀʏ ᴛɪᴍᴇ : %I:%M:%S %p")         
            await message.reply_text(f"#ᴘʀᴇᴍɪᴜᴍ_ᴀᴅᴅᴇᴅ_ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ✅\n\n◉ ᴜꜱᴇʀ : {user.mention}\n◉ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n◉ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n\n◉ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n◉ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}", disable_web_page_preview=True)
            await client.send_message(
                chat_id=user_id,
                text=f"ʜᴇʏ {user.mention},\nᴛʜᴀɴᴋ ʏᴏᴜ ꜰᴏʀ ᴘᴜʀᴄʜᴀꜱɪɴɢ ᴘʀᴇᴍɪᴜᴍ.\nᴇɴᴊᴏʏ !! 🎉\n\n◉ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n◉ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n◉ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}", disable_web_page_preview=True              
            )    
            await client.send_message(PREMIUM_LOGS, text=f"#ᴀᴅᴅᴇᴅ_ᴘʀᴇᴍɪᴜᴍ\n\n◉ ᴜꜱᴇʀ : {user.mention}\n◉ ᴜꜱᴇʀ ɪᴅ : <code>{user_id}</code>\n◉ ᴘʀᴇᴍɪᴜᴍ ᴀᴄᴄᴇꜱꜱ : <code>{time}</code>\n\n◉ ᴊᴏɪɴɪɴɢ ᴅᴀᴛᴇ : {current_time}\n\n◉ ᴇxᴘɪʀʏ ᴅᴀᴛᴇ : {expiry_str_in_ist}", disable_web_page_preview=True)
...
@Client.on_message(filters.command("reset_trial"))
async def reset_trial(client, message):
    user_id = message.from_user.id
    if user_id not in ADMINS:
        await message.reply("ʏᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴀɴʏ ᴘᴇʀᴍɪꜱꜱɪᴏɴ ᴛᴏ ᴜꜱᴇ ᴛʜɪꜱ ᴄᴏᴍᴍᴀɴᴅ ‼️")
        return
